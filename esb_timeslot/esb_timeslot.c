...
/** Address. */
static const uint8_t base_addr_0[4] = {ADDR_PIPE0_1, ADDR_PIPE0_2, ADDR_PIPE0_3, ADDR_PIPE0_4};     /**< Base address part #0. */
static const uint8_t base_addr_1[4] = {ADDR_PIPE1TO5_1, ADDR_PIPE1TO5_2, ADDR_PIPE1TO5_3, ADDR_PIPE1TO5_4};     /**< Base address part #1. */
static const uint8_t addr_prefix[8] = {ADDR_PIPE0_0, ADDR_PIPE1_0, ADDR_PIPE2_0, ADDR_PIPE3_0,
																			ADDR_PIPE4_0, ADDR_PIPE5_0, 0xC7, 0xC8 };    /**< Address prefix. */
...
void TIMESLOT_BEGIN_IRQHandler(void)
{
    ...
    if (m_state == STATE_IDLE)
    {
        ...
        APP_ERROR_CHECK(err_code);
#if !defined(_INTACT)			
				err_code = nrf_esb_set_rf_channel(ESB_RF_CHANNEL);
				APP_ERROR_CHECK(err_code);
#endif			
    }
...
uint32_t esb_timeslot_send_data(uint8_t pipe, uint8_t * p_str, uint8_t len)
{
    static nrf_esb_payload_t tx_payload;
    bool success;

    memset(&tx_payload, 0, sizeof(tx_payload));
    memcpy(tx_payload.data, p_str, len);
    tx_payload.length = len;
    tx_payload.pipe   = pipe;

	
    CRITICAL_REGION_ENTER();
    success = fifo_put_pkt(&m_transmit_fifo, 
			(uint8_t *)&tx_payload, sizeof(tx_payload));
    CRITICAL_REGION_EXIT();
    
    return (success? NRF_SUCCESS: NRF_ERROR_NO_MEM);
}/*esb_timeslot_send_data*/
...
#define ESB_DATA_RATE									(NRF_ESB_BITRATE_1MBPS)	
		nrf_esb_config.bitrate            = ESB_DATA_RATE;
#else	
