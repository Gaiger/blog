...
extern void app_usbd_msc_read_callback(uint32_t number_of_block);
extern void app_usbd_msc_write_callback(uint32_t number_of_block);
/**
  * @brief  .
  * @param  lun: .
  * @retval USBD_OK if all operations are OK else USBD_FAIL
...
{
  /* USER CODE BEGIN 6 */
	app_usbd_msc_read_callback(blk_len);
...
{
  /* USER CODE BEGIN 7 */
	app_usbd_msc_write_callback(blk_len);
	PRINTF("%s :: blk_addr = %lu, blk_len = %u\r\n", __FUNCTION__, blk_addr, blk_len);
